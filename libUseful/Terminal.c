...
void TerminalMenuDraw(STREAM *Term, ListNode *Options, int x, int y, int wid, int len)
{
		...
		Curr=ListGetNext(Options);
		while (Curr)
		{
					TerminalCursorMove(Term, x, y);
					if (Options->Side==Curr)
					{
						  STREAMWriteString(ANSICode(ANSI_BLACK,ANSI_WHITE,0), Term);
						  STREAMWriteString("> ", Term);
					}
					else
					{
						  STREAMWriteString(ANSICode(ANSI_BLACK,ANSI_CYAN,0), Term);
						  STREAMWriteString("  ", Term);
					}

					STREAMWriteString(Curr->Tag, Term);
					count=StrLen(Curr->Tag);
					while (count < wid)
					{
							STREAMWriteChar(Term,' ');
							count++;
					}

					if (Options->Side==Curr) STREAMWriteString(" <", Term);
					else STREAMWriteString("  ", Term);
					STREAMWriteString(ANSI_NORM, Term);
					y++;
					if (y > yend) break;
					Curr=ListGetNext(Curr);
		}
...
ListNode *TerminalMenu(STREAM *Term, ListNode *Options, int x, int y, int wid, int len)
{
int result;

Options->Side=ListGetNext(Options);
TerminalMenuDraw(Term, Options, x, y, wid, len);

result=TerminalReadChar(Term);
while (1)
{
	switch (result)
	{
		case KEY_UP:
				if (Options->Side) 
				{
					if (Options->Side->Prev && (Options->Side->Prev != Options)) Options->Side=Options->Side->Prev;
				}
				else Options->Side=ListGetNext(Options);
			 break;

		case KEY_DOWN:
				if (Options->Side) 
				{
					if (Options->Side->Next) Options->Side=Options->Side->Next;
				}
				else Options->Side=ListGetNext(Options);
			 break;
