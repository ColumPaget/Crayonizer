...
static const char *ParseStringAction(const char *Line, int Type, TCrayon *Crayon)
{
    ...
    for (sptr=Token; *sptr != '\0'; sptr++)
    {
        if (*sptr=='\\')
        {
            ...
            {
            case 'a':
                Action->String=AddCharToStr(Action->String,0x07);
                break;
            case 'b':
                Action->String=AddCharToStr(Action->String,0x08);
                break;
            case 'c':
                sptr++;
                ...
                break;
            case 'n':
                Action->String=AddCharToStr(Action->String,'\n');
                break;
            case 'r':
                Action->String=AddCharToStr(Action->String,'\r');
                break;
            case 't':
                Action->String=AddCharToStr(Action->String,'	');
                break;
            default:
                Action->String=AddCharToStr(Action->String, *sptr);
                break;
            }
        }
...
//Do this before anything else!
    ...
    else
    {
        ...
        switch (val)
        {
        case ACTION_CLRTOEOL:
            (*Action)->Attribs |= FLAG_CLR2EOL;
            break;
        ...
        default:
            (*Action)=NewCrayonAction(Crayon, val);
            break;
        }
    }
...
const char *ParseCrayonAction(TCrayon *Item, const char *Args)
{
    ...
    case CRAYON_SECTION:
        Item->Start=strtol(Item->Match,&wptr,10);
        wptr++;
        Item->Len=strtol(wptr,NULL,10) - Item->Start;
